...
async function run(
  ...
  // Run encode-bundle cli
  const { exitCode, stdout, stderr } = await execa(bin, [...entry, ...(options.flags || [])], {
    cwd: testDir,
...
test('declaration files with multiple entrypoints #316', async () => {
  ...
  expect(await getFileContent('dist/index.d.ts'), 'dist/index.d.ts').toMatchSnapshot();
  expect(await getFileContent('dist/bar/index.d.ts'), 'dist/bar/index.d.ts').toMatchSnapshot();
});
...
test('custom output extension', async () => {
  const { outFiles } = await run(
    getTestName(),
    {
      ...
      'encode-bundle.config.ts': `export default {
        outExtension({ format }) {
          return {
            js: '.' + format + '.js'
          }
